...
def twoModesDistance():
    ...
    totList = [('an average', 'rate', rateTot, rateUnit, 'of'), ('in total', 'time', timeTot, timeUnit, 'for'), ('in total', 'dist', distTot, distUnit, 'for')]
    ...
    # and "parameters": rate, time, and distance (where we tend to exclude using "rate + total" given real number
    ...
    # Therefore, we start by picking the mode (including possibly 'total') from which we will provide two of the three parameters
    ...
    # We do this by ensuring that we don't pick the answer/question from the mode that we also assigned for two-parameters within assumptions
    ansChoices = [0, 1, 2]
